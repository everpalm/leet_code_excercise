...
class TimeMap:
    ...
    def set(self, key, value, timestamp):
        # If key is not in store, initialize it with an empty list
        if key not in self.store:
            self.store[key] = []
        # Append the (timestamp, value) to the list associated with the key
    ...
    def get(self, key, timestamp):
        ...
        print('i = ', i)
        # If i is 0, it means all timestamps are greater than the given timestamp
        if i == 0:
            return ""
...
def test_time_map():
    ...
    for i, operation in enumerate(operations):
        if operation == "TimeMap":
            ...
            # Destructure assignment (multiple assignment)
            key, value, timestamp = arguments[i]
            time_map.set(key, value, timestamp)
            output.append(None)
        elif operation == "get":
            key, timestamp = arguments[i]
