...
def remove_element(nums, val):
    ...
    while i < len(nums):
        if nums[i] == val:
            ...
            i+=1
    return len(nums)
    # i = 0  # Initialize index to start from the beginning of the list
    # while i < len(nums):  # Iterate through the list
    #     if nums[i] == val:  # Check if the current element is the target value
    #         nums.pop(i)  # Remove the element at the current index
    #         # Do not increment i, as we want to check the new element that shifted to this index
    #     else:
    #         i += 1  # Move to the next element if the current is not val
    # return len(nums)  # Return the new length of the list after all removals

# Let's not call the function here to avoid executing it in this environment. We'll discuss this next.
